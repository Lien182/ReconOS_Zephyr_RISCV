...
def _export_hw_vivado(prj, hwdir, link):
	...
	if hasattr(prj.impinfo, 'constraints'):
		if not shutil2.isfile(prj.impinfo.constraints):
			log.warning("No Constraints file specified. Implementation could fail")
		else:
			print("Copying constraints file to the hardware directory")
			shutil2.copytree(prj.impinfo.constraints, hwdir)
