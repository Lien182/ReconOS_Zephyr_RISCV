...
def cmake(filepath1, project): 
	swdir = re.sub(r'CMakeLists.txt', '', filepath1)
	string = ''
	...
	with open(filepath1, 'r') as file:
		try:
			...
			return
	for files in shutil2.listfiles(swdir, True, "c[p]*$"):
		if re.match("^rt", files) is not None:
			string += files + ' '	

	if '<<projectname>>' not in data:
		return
	else:
		data = re.sub(r"<<projectname>>", project.name, data)

	if '<<softwarethread>>' not in data:
		return
	else:
		data = re.sub(r"<<softwarethread>>", string, data)
