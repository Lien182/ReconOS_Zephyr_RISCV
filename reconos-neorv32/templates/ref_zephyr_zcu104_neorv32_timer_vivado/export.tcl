...
proc reconos_hw_setup {new_project_name new_project_path reconos_ip_dir} {
    ...
    set_property "board_part" "xilinx.com:zcu104:part0:1.1" $proj_name
    ...
    ## Constraints
    set fileset_constraints $proj_dir/zcu104-Master.xdc
    ...
    if {[string equal [get_runs -quiet synth_1] ""]} {
        create_run -name synth_1 -part xczu7ev-ffvc1156-2-e -flow {Vivado Synthesis 2020} -strategy "Vivado Synthesis Defaults" -constrset constrs_1
    } else {
    ...
    if {[string equal [get_runs -quiet impl_1] ""]} {
        create_run -name xczu7ev-ffvc1156-2-e -flow {Vivado Implementation 2020} -strategy "Vivado Implementation Defaults" -constrset constrs_1 -parent_run synth_1
    } else {
...
set_property -dict [list CONFIG.IO_TWI_EN {false} CONFIG.IO_WDT_EN {false} CONFIG.IO_TRNG_EN {false} CONFIG.IO_NEOLED_EN {false} CONFIG.CUSTOM_ID {0x000000FF} CONFIG.MEM_INT_IMEM_SIZE {131072} CONFIG.MEM_INT_DMEM_SIZE {65536} CONFIG.XIRQ_NUM_CH {32}] [get_bd_cells processing_system7_0]
...
connect_bd_net [get_bd_pins xlconstant_3/dout] [get_bd_pins processing_system7_0/msw_irq_i]
connect_bd_net [get_bd_pins processing_system7_0/mext_irq_i] [get_bd_pins xlconstant_3/dout]
...
set_property -dict [list CONFIG.PRIMITIVE {PLL} CONFIG.CLK_IN1_BOARD_INTERFACE {clk_300mhz} CONFIG.CLKOUT2_USED {true} CONFIG.CLK_OUT1_PORT {main_out} CONFIG.CLK_OUT2_PORT {FCLK_0} CONFIG.USE_LOCKED {false} CONFIG.USE_RESET {false} CONFIG.PRIM_SOURCE {Differential_clock_capable_pin} CONFIG.CLKIN1_JITTER_PS {33.330000000000005} CONFIG.CLKOUT1_DRIVES {Buffer} CONFIG.CLKOUT2_DRIVES {Buffer} CONFIG.CLKOUT3_DRIVES {Buffer} CONFIG.CLKOUT4_DRIVES {Buffer} CONFIG.CLKOUT5_DRIVES {Buffer} CONFIG.CLKOUT6_DRIVES {Buffer} CONFIG.CLKOUT7_DRIVES {Buffer} CONFIG.MMCM_DIVCLK_DIVIDE {3} CONFIG.MMCM_BANDWIDTH {OPTIMIZED} CONFIG.MMCM_CLKFBOUT_MULT_F {8} CONFIG.MMCM_CLKIN1_PERIOD {3.333} CONFIG.MMCM_CLKIN2_PERIOD {10.0} CONFIG.MMCM_COMPENSATION {AUTO} CONFIG.MMCM_CLKOUT0_DIVIDE_F {8} CONFIG.MMCM_CLKOUT1_DIVIDE {8} CONFIG.NUM_OUT_CLKS {2} CONFIG.CLKOUT1_JITTER {143.207} CONFIG.CLKOUT1_PHASE_ERROR {114.212} CONFIG.CLKOUT2_JITTER {143.207} CONFIG.CLKOUT2_PHASE_ERROR {114.212}]  [get_bd_cells clk_wiz_0]
...
set_property name dram [get_bd_cells blk_mem_gen_0]
set_property -dict [list CONFIG.PRIM_type_to_Implement {URAM} CONFIG.Assume_Synchronous_Clk {true} CONFIG.EN_SAFETY_CKT {false}] [get_bd_cells dram]
connect_bd_intf_net [get_bd_intf_pins dmem/BRAM_PORTA] [get_bd_intf_pins dram/BRAM_PORTA]

create_bd_cell -type ip -vlnv xilinx.com:ip:axi_bram_ctrl:4.1 axi_bram_ctrl_1
set_property name imem [get_bd_cells axi_bram_ctrl_1]
set_property -dict [list CONFIG.PROTOCOL {AXI4LITE} CONFIG.ECC_TYPE {0} CONFIG.SINGLE_PORT_BRAM {1}] [get_bd_cells imem]

create_bd_cell -type ip -vlnv xilinx.com:ip:blk_mem_gen:8.4 blk_mem_gen_1
set_property name iram [get_bd_cells blk_mem_gen_1]
set_property -dict [list CONFIG.PRIM_type_to_Implement {URAM} CONFIG.Assume_Synchronous_Clk {true} CONFIG.EN_SAFETY_CKT {false}] [get_bd_cells iram]
connect_bd_intf_net [get_bd_intf_pins imem/BRAM_PORTA] [get_bd_intf_pins iram/BRAM_PORTA]
...
make_bd_pins_external  [get_bd_pins clk_wiz_0/clk_in1_n]
set_property name clk_in1_n [get_bd_ports clk_in1_n_0]
make_bd_pins_external  [get_bd_pins clk_wiz_0/clk_in1_p]
set_property name clk_in1_p [get_bd_ports clk_in1_p_0]
...
connect_bd_net [get_bd_pins clk_wiz_0/main_out] [get_bd_pins processing_system7_0/main_clk]
    ...
    create_bd_cell -type ip -vlnv xilinx.com:ip:axi_interconnect:2.1 axi_dmem

    set_property -dict [ list CONFIG.NUM_MI {1}  CONFIG.NUM_SI {1} ] [get_bd_cells axi_mem]
    set_property -dict [ list CONFIG.NUM_MI {5}  ] [get_bd_cells axi_hwt]
    set_property -dict [ list CONFIG.NUM_MI {3} CONFIG.NUM_SI {2}  ] [get_bd_cells axi_dmem]

    # PS memory interface 
    connect_bd_intf_net [get_bd_intf_pins processing_system7_0/m_axi] [get_bd_intf_pins axi_dmem/S00_AXI]
    connect_bd_intf_net -intf_net axi_dmem_M00_AXI [get_bd_intf_pins axi_dmem/M00_AXI] [get_bd_intf_pins imem/S_AXI]
    connect_bd_intf_net -intf_net axi_dmem_M01_AXI [get_bd_intf_pins axi_dmem/M01_AXI] [get_bd_intf_pins dmem/S_AXI]
    ...
    connect_bd_net [get_bd_pins clk_wiz_0/main_out] [get_bd_pins axi_dmem/M01_ACLK] 
    connect_bd_net [get_bd_pins clk_wiz_0/main_out] [get_bd_pins imem/s_axi_aclk]
    ...
    connect_bd_net [get_bd_pins reset_1/peripheral_aresetn] \
                            ...
                            [get_bd_pins imem/s_axi_aresetn]
    ...
    connect_bd_intf_net -intf_net axi_hwt_S00_AXI [get_bd_intf_pins axi_hwt/S00_AXI] [get_bd_intf_pins axi_dmem/M02_axi] 
    ...
    connect_bd_intf_net -intf_net axi_dmem_S01_AXI [get_bd_intf_pins axi_dmem/S01_AXI] [get_bd_intf_pins axi_mem/M00_AXI]
    ...
    connect_bd_net [get_bd_pins reset_0/Interconnect_aresetn] \
                            ...
                            [get_bd_pins axi_dmem/M02_ARESETN]
    ...
    connect_bd_net [get_bd_pins xlconstant_2/dout] [get_bd_pins xlconcat_1/In14]
    connect_bd_net [get_bd_pins xlconstant_2/dout] [get_bd_pins xlconcat_1/In15]
    ...
    connect_bd_net [get_bd_pins reconos_osif_intc_0/OSIF_INTC_Out] [get_bd_pins xlconcat_1/In0]
    connect_bd_net [get_bd_pins reconos_proc_control_0/PROC_Pgf_Int] [get_bd_pins xlconcat_1/In1]
    ...
    #Change the size of the size of the memory as desired by setting the address range. 
    set_property range 512K [get_bd_addr_segs {reconos_memif_memory_controller_0/M_AXI/SEG_dmem_Mem0}]    
    set_property offset 0x80000000 [get_bd_addr_segs {processing_system7_0/m_axi/SEG_dmem_Mem0}]
    set_property range 512K [get_bd_addr_segs {processing_system7_0/m_axi/SEG_dmem_Mem0}]

    #Change the size of the size of the memory as desired by setting the address range.
    assign_bd_address [get_bd_addr_segs {imem/S_AXI/Mem0 }]
    set_property range 512K [get_bd_addr_segs {processing_system7_0/m_axi/SEG_imem_Mem0}]
    set_property offset 0x00000000 [get_bd_addr_segs {processing_system7_0/m_axi/SEG_imem_Mem0}]
    set_property range 512K [get_bd_addr_segs {reconos_memif_memory_controller_0/M_AXI/SEG_imem_Mem0}]
    set_property offset 0x00000000 [get_bd_addr_segs {reconos_memif_memory_controller_0/M_AXI/SEG_imem_Mem0}]
    
    exclude_bd_addr_seg [get_bd_addr_segs reconos_memif_memory_controller_0/M_AXI/SEG_imem_Mem0]
    
    set_property -dict [list CONFIG.MEM_INT_DMEM_EN {false} CONFIG.MEM_INT_IMEM_EN {false}] [get_bd_cells processing_system7_0]
