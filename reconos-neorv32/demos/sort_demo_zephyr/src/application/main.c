...
void main()
{
	...
	log("creating %d sw-threads:", num_swts);
	...
	data = (uint32_t *)k_malloc(data_count * sizeof(uint32_t));
	copy = (uint32_t *)k_malloc(data_count * sizeof(uint32_t));
	...
	if (data!=NULL && copy!=NULL){
		...
		log("Memory could not be allocated. Exiting...\n");
		while (1){
