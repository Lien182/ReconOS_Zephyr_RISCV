...
//Printing is disabled to obtain the results in the thesis work. 
#ifdef PRINT_ENABLE
	#define printlog(...) printf(__VA_ARGS__); fflush(stdout)
#else
	#define printlog(...)
#endif
// mailboxes
...
void main() {
	...
	int hw_threads = 1;			//2 hardware threads and 0 software threads work only when RECONOS_DEBUG is defined in $RECONOS_DIR/templates/app_zephyr/lib/runtime/utils.h. Otherwise the main thread is stuck in an infinite loop (maybe due to the priority of the main thread?) 
	int sw_threads = 0;
	int str_matrix_size = 16;
	...
	printlog("Size of mailboxes: %i\n", mbox_size);
	printlog("Address of mb_start: %p\n", &mb_start);
	...
	printlog("Generating input data.\n");
	...
	printlog("Generating check results.\n");
	...
	// init hw-threads
	printlog("Creating %i hw-thread(s).\n", hw_threads);
	...
	// init sw-threads
	printlog("Creating %i software threads.\n", sw_threads);
	...
	printlog("Running Strassen algorithm part 1 - split.\n");
	...
	printf("Threads have been terminated.\n");

	// combine results (strassen algorithm part 2)
	printlog("Running Strassen algorithm part 2 - combine.\n");
...
// #endif
	...
	printf("==================\n");
	printf("Generate input data:   %f ms\n", timer_toms(generate_data_time));
	printf("Generate check result: %f ms\n", timer_toms(generate_check_result_time));
	printf("Initializing HWT:      %f ms\n", timer_toms(init_hwt_time));
	printf("Initializing SWT:      %f ms\n", timer_toms(init_swt_time));
	printf("Str. split (part 1):   %f ms\n", timer_toms(str_mmp_split));
	printf("Std. MMP:              %f ms\n", timer_toms(std_mmp_time));
	printf("Str. combine (part 2): %f ms\n", timer_toms(str_mmp_combine));
	printf("~Thread term. HWT:     %f ms\n", timer_toms(terminate_hwt_time));
	printf("~Thread term. SWT:     %f ms\n", timer_toms(terminate_swt_time));
	printf("Check HWT result:      %f ms\n\n", timer_toms(comparision_time));
	printf("Important timing results\n");
	printf("========================\n");
	printf("Runtime Std. MMP:      %f ms\n", timer_toms(calculation_time_std));
	printf("Runtime Str. MMP:      %f ms\n", timer_toms(calculation_time_str));
